...
function addSp(amount)
    skill_points = math.max(math.min(skill_points + amount, max_sp), 0)
end
...
function SkillPoint:die()
    ...
    self.area:addGameObject('InfoText', self.x + table.random({-1, 1})*self.w, self.y + table.random({-1, 1})*self.h, {text = '+1 SP', color = skill_point_color})
end
