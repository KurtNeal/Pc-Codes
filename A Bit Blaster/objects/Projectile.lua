...
function Projectile:new(area, x, y, opts)
    ...
    if self.attack == '2Split' then
        self.timer:every(0.02, function()
            local r = Vector(self.collider:getLinearVelocity()):angle()
            self.area:addGameObject('TrailParticle', self.x - 1.0*self.s*math.cos(r), self.y - 1.0*self.s*math.sin(r), 
            {parent = self, r = random(1, 3), d = random(0.1, 0.15), color = ammo_color}) 
        end)
    end
...
function Projectile:update(dt)
    ...
    if self.attack == '2Split' then
        ...
        if self.y < 0 then
            self:die()
            self.area:addGameObject('Projectile', self.x + 1.5*d*math.cos(math.pi/4), self.y + 1.5*d*math.sin(math.pi/4), {r = math.pi/4, attack = 'Neutral', bounce = 0})
            self.area:addGameObject('Projectile', self.x + 1.5*d*math.cos(3*math.pi/4), self.y + 1.5*d*math.sin(3*math.pi/4), {r = 3*math.pi/4, attack = 'Neutral', bounce = 0})
        end
        if self.y > gh then
            self:die()
            self.area:addGameObject('Projectile', self.x + 1.5*d*math.cos(self.r + 5*math.pi/4), self.y - 1.5*d*math.sin(self.r + 5*math.pi/4), {r = 5*math.pi/4, attack = 'Neutral', bounce = 0})
            self.area:addGameObject('Projectile', self.x + 1.5*d*math.cos(self.r + 7*math.pi/4), self.y - 1.5*d*math.sin(self.r + 7*math.pi/4), {r = 7*math.pi/4, attack = 'Neutral', bounce = 0})
        end
    end
